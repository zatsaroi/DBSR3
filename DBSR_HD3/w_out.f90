!======================================================================
      Subroutine W_out
!======================================================================
!     calculates channels and pertuber contributions for each solution;
!     these weights are recorded in file w.out (if iwt >0 and
!     printed in the log-file if cwt > 0;
!     they are also used to provide the solution assognments
!----------------------------------------------------------------------
...
!
!----------------------------------------------------------------------
!     Using squire-root matrix:  (used in previous version)
!
...
!
!-----------------------------------------------------------------------
!     Using Cholesky decomposition:  (present implimentation)
!
!     S = L L' ->     H C = L L' C E
!                    [inv(L) H inv(L')] L'C = L'C E
!                    (L'C)' (L'C) = 1
!
!     Solutions are given as (L'C) in matrix A from provious step.
!-----------------------------------------------------------------------
      Use dbsr_hd

      Implicit none
      Integer :: i,i1,i2,is,mwt
...
! ... define and record weights:
      ...
      Do is = 1,khm
       ...
       Do i = 1,nch
        i1=ipsol(i-1)+1; i2=ipsol(i); WT(i)=SUM(cval(i1:i2))
       End do
