...
! ... find the occupation numbers for all orbitals:
      ...
      S_orb = 0.d0; SS = 0.d0
      Do jc = 1,1 !  ncfg                            !!!
       ic = ipt(jc); S = WC(ic)*WC(ic);   S = 1.d0   ! ???
       SS = SS + S
       Call Get_cfg_jj(ic)
       ip = ip_state(ic)
       if(jc.eq.1) S = 1.d0  ! the fist configuration should be included anyway
       Do i=1,no; ip=ip+1; ii=ipef(ip_orb(ip))
        S_orb(ii) = S_orb(ii) + iq(i)*S
       End do
       if(SS.gt.eps_conf) Exit
      End do
      S_orb = S_orb / SS
...
! ... choose the substitution orbital with biggest overlap:
      ...
      write(muc,'(a,20x,3f10.3)') '*'
      write(nuo,'(a,20x,3f10.3)') '*'
