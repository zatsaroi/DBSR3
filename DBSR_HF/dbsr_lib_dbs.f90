...
! ... read parameters from file:
      ...
      if(rrms.le.0.d0.and.atw.ne.0.d0) then
       if(an.le.90)  then
...
! ... create the knot grid:      Correction: Apr.9 2020  (awt question after A.Kramida)

      if(nug.eq.0) then
       if(len_trim(name).ne.0) AF_grid = trim(name)//'.knot'
