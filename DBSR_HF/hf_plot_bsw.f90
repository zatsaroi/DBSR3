...
! ... cycle over orbitals
      ...
      Do io=1,nsol_nl
       if(nbs(nwf)+io-1.gt.99) Exit
       yp = 0.d0
       yq = 0.d0
       Do i = 2,np-1
        yp(i) =  bvalu2 (tp, p_nl(1,io), nsp, ksp, r(i), 0)
        yq(i) =  bvalu2 (tq, p_nl(ns+1,io), nsq, ksq, r(i), 0)
        nr = i; if(r(i+1).gt.r_max) Exit
       End do
       nr = nr + 1

       gamma = lbs(nwf) + 1
       if(nuclear.eq.'point') gamma = sqrt (kbs(nwf)**2 - (z/c_au)**2)
       P0 = yp(2)/r(2)**gamma
       write(nuw) nbs(nwf)+io-1,kbs(nwf),e_nl(io),nr
       write(nuw) P0,yp(1:nr),yq(1:nr)  
       write(nuw) r(1:nr)
